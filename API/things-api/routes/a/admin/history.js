  ...
  ****************************************************/
  timespan: (req,res)=>{
    res.app.locals.pool.connect(function(err, client, done) {
        ...
        client.query('SELECT * FROM transactions AS t LEFT JOIN items AS i ON t.item_id = i.item_id WHERE date(timestamp) <= $2 AND date(timestamp) >= $1', [start_date, end_date], function(err, result) {
            done();
